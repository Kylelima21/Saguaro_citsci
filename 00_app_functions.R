...
#' Function to download photos of 9 random iNaturalist observations
#'
#' This function takes a data frame of iNaturalist records (created specifically for the
#' output of the "inat_recent()" function) and downloads the images associated with 9 random and
#' unique species.
#'
#' @inheritParams None
#' @param x: Data frame of iNaturalist observations.
...
download_photos <- function(x, output.path) {
  ...
  sample_size <- if(nrow(pic_data) > 9) { 
    9
  } else { 
    nrow(pic_data) 
  }
  ...
  loop_list <- pic_9random %>% 
    select(image.url) %>% 
    unlist()
  
  # Create the list of names for the photos
  loop_filenames <- pic_9random %>% 
    select(pic.name) %>% 
    unlist()
  ...
  write.csv(pic_9random, "www/datasets/summary_9random.csv", row.names = F)
